...
export class HomeService {
  ...
  getBanners(): Observable<Banner[]> {
    return this.http.get<{ banners: Banner[] }>( 'http://localhost:3000/banner')
      .pipe(map(res => res.banners));
  }

  getHotTags(): Observable<HotTag[]> {
    return this.http.get<{ tags: HotTag[] }>('http://localhost:3000/playlist/hot')
      .pipe(map(res => {
  ...
  getPersonalSongPlaylist(): Observable<SongPlaylist[]> {
    return this.http.get<{ result: SongPlaylist[] }>('http://localhost:3000/personalized')
